...
class AncientSword(BasicArtifact):
    ...
    def activate(self, character):
        if hasattr(character, 'attack_power'):
            ...
            print(f"{self.name} equipped by {character.name}. Attack power increased by {self.power}.")
        else:
            print(f"{self.name} cannot be used by {character.name} as they lack 'attack_power' attribute.")
